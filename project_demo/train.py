...
def run_training():
    ...
    input_data = tf.placeholder(tf.int32, [FLAGS.batch_size, None])  #TensorFlow中的占位符，用于传入外部数据。
    output_data = tf.placeholder(tf.int32, [FLAGS.batch_size, None])

    end_points = rnn_model(model='lstm', input_data=input_data, output_data=output_data, vocab_size=len(
    ...
    with tf.Session() as sess:
        ...
        if checkpoint:
            saver.restore(sess, checkpoint)  #在最后时刻保存的模型基础上进行训练
            print("## restore from the checkpoint {0}".format(checkpoint))
            start_epoch += int(checkpoint.split('-')[-1])  #分批处理，可参照model下的序号
        print('## start training...')
        try:
            n_chunk = len(poems_vector) // FLAGS.batch_size  #分批次处理训练集向量
